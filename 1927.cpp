...
int pop(heap* H) {
    int top = H->heap[1].key; //반환할 루트 값 (최소값)
    ...
    while (H->heapSize >= child) { 
        ...
        if (child < H->heapSize && H->heap[child].key > H->heap[child+1].key) { 
            //왼쪽 자식이 오른쪽 자식보다 작은 경우 child ++ 을 해서 오른쪽 자식으로 이동하기
            child++;
        }
        if (temp.key < H->heap[child].key) { //temp가 child보다 작으면 그만 바꿔도 됨
            break;
        }
        //아닌 경우에는 바꿔줘야 함
